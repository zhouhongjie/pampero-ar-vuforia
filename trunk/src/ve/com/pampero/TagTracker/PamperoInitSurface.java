 ...
 */
public class PamperoInitSurface extends GLSurfaceView {
    private static boolean mUseOpenGLES2 = true;

    /** Constructor. */
    public PamperoInitSurface(Context context)
    {
        super(context);
    }
    ...
    public void init(int flags, boolean translucent, int depth, int stencil)
    {
        ...
        LoggerPrint.INFO("Using OpenGL ES " + (mUseOpenGLES2 ? "2.0" : "1.x"));
        LoggerPrint.INFO("Using " + (translucent ? "translucent" : "opaque") +
    ...
    private static class ContextFactory implements EGLContextFactory
    {
                                        ...
                                        EGLConfig eglConfig)
        {
            ...
            if (mUseOpenGLES2)
            {
                LoggerPrint.INFO("Creating OpenGL ES 2.0 context");
            ...
            {
                LoggerPrint.INFO("Creating OpenGL ES 1.x context");
                checkEglError("Before eglCreateContext", egl);
    ...
    private static void checkEglError(String prompt, EGL10 egl)
    {
        ...
        {
            LoggerPrint.ERROR(String.format("%s: EGL error: 0x%x", prompt, error));
        }
    }
